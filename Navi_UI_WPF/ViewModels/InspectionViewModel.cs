...
{
    /// <summary>
    /// ViewModel cho Inspection View - Kế thừa từ BaseAssemblyViewModel
    /// </summary>
    public class InspectionViewModel : BaseAssemblyViewModel
    {
        ...
        public override async Task LoadProductAssemblyAsync(string productName, string po = null)
        {
            ...
            try
            {
                ...
                {
                    ErrorMessage = $"Không tìm thấy dữ liệu kiểm tra cho sản phẩm: {productName}";
                    return;
                }

                ProductId = 0; 
                ProductName = productName;
                Description = $"Quy trình kiểm tra cho {productName}";
            ...
            {
                Log.Error(ex, "Error loading inspection items for {ProductName}", productName);
                ErrorMessage = $"Lỗi khi tải dữ liệu kiểm tra: {ex.Message}";
            }
            finally
            {
                IsLoading = false;
        ...
        protected override async Task RecordStepHistoryAsync(bool isOk = false, bool isNg = false, string note = null)
        {
            ...
            {
                Log.Error(ex, "Error recording history for inspection step {Step}", CurrentStep?.StepNumber);
            }
        }
        ...
        private async Task SearchPoAsync()
        {
            ...
            {
                Log.Error(ex, "Error in Inspection SearchPoAsync");
